...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Dynamic rendering logic (as per the new requirements)
  const dynamicEventGrid = document.getElementById("eventGrid");
  if (dynamicEventGrid) {
    // Clear existing content
    dynamicEventGrid.innerHTML = "";

    // Render events dynamically (this could be from a different data source or API)
    const events = [
      {
        image: "images/3dikgosi.jpg",
        title: "Gaborone Food Festival",
        date: "July 21, 2025",
        location: "National Stadium, Gaborone",
        description: "The number one Music Festival.",
        detailsUrl: "event-details.html",
        signupUrl: "signup.html",
        signupText: "Sign up to volunteer",
      },
      {
        image: "images/mshati.jpg",
        title: "Dikgosi tse Tharo",
        date: "August 23, 2025",
        location: "Gaborone Magistrate, CBD",
        description: "The Three Kings who liberated Botswana from Itself.",
        detailsUrl: "event-details.html",
        signupUrl: "signup.html",
        signupText: "Sign up as a volunteer",
      },
    ];

    // Map events to HTML and set innerHTML (safer than directly injecting user data)
    dynamicEventGrid.innerHTML = events
      .map(
        (event) => `
      <article class="event-card">
        ...
        <div class="card-content">
          ...
          <a href="${event.detailsUrl}" class="card-button">View Details</a>
          <a href="${event.signupUrl}" class="signup-btn">${event.signupText}</a>
        </div>
      </article>
    `
      )
